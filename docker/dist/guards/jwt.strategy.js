...
let JwtStrategy = class JwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy, 'jwt-strategy-user') {
    ...
    async validate(payload) {
        const user = await this.userService.findUserBy({ id: payload.id, isActive: enums_1.Status.Active });
        if (!user) {
            throw new common_1.UnauthorizedException('Invalid Authorization key');
        }
        return {
            id: payload.id,
            name: (0, functions_helpers_1.getFullName)(user),
