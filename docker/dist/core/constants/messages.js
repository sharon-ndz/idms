...
exports.MESSAGES = {
    ...
    recordInUse: (record) => `${record} already used`,
    recordCountDelete: (count, max) => `${count} of ${max} record(s) delete`,
    ...
    docDescription: "The Integrated Driver's License Management System (IDLMS) is a comprehensive digital platform designed to streamline vehicle registration, driver licensing, and related services in Lagos State",
    ...
    passwordStrengthFailed: 'Password At least one uppercase letter, At least one lowercase letter, At least one digit (number), At least one special character, At least 8 characters in total.',
    ...
    invalidValue: (token) => `Invalid ${token}`,
    otpEmailBody: (otpCode) => `<p>Dear User,</p>
    ...
    <p>Best regards,<br>`,
    userAccountCreated: (password, email) => `<p>An account has been created for you by the administrator of the system.</p> 
    <p>Use the following details for your user access:  <br/> Email: <b>${email}</b><br/>Password: <b>${password}</b></p>
    <p>Best regards,<br> `,
    drivingSchoolApplicationCreated: (name, email, password) => `<p>Dear ${name},</p>
    ...
    <p>Best regards,<br> `,
    preRegistrationEmailBody: (applicationNo, name) => `<p>Dear ${name},</p>
    ...
    <p>Thank you for choosing our service.</p>
    <p>Best regards,<br>`,
    licenseEmailBody: (applicationNo, licenseNo, name) => `<p>Dear ${name},</p>
    ...
    newApplicationAssignedEmailBody: (name, identifier) => `<p>Dear ${name},</p>
    <p>You have been assigned to review the application for driving school with identifier number: <strong>${identifier}</strong>.</p>`,
    applicationApprovedEmailBody: () => `<p>Congratulations</p>
    <p>You have been assigned a LASDRI officer for the inspection.</p>`,
    applicationPaymentConfirmedEmailBody: (schoolName, schoolEmail, schoolPhone) => `<p>Dear User,</p>
